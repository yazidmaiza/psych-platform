...
exports.endSession = async (req, res) => {
  try {
    ...
    if (history.length === 0) return res.status(400).json({ message: 'No conversation to summarize' });
    const conversationText = history.map(msg => `: `).join('\n');
    ...
    const summary = await ChatbotSummary.create({
      ...
      emotionalIndicators: { dominantEmotion: parsed.dominantEmotion, urgencyScore: parsed.urgencyScore, sentimentTrend: parsed.sentimentTrend },
